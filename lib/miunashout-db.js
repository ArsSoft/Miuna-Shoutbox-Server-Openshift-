...
exports.getOldMsgsGuest = function(data, cb){
	...
	var query = Chat.find({}).or([{ type: 'shout'}, { type: 'system' }]);
	query.sort('-_id').limit(data.ns).exec(function(err, docs){
		cb(err, docs);
	});
}
...
exports.updmsg = function(data, cb){
	Chat.findOne({_id: data.id}).exec(function(err, docs){
		if ((docs.uid==data.tk_uid && data.tk_edtp=='1') || data.tk_mod=='1') {
			Chat.findOneAndUpdate({_id: data.id}, { msg: xss(data.newmsg), edt: '1', edtusr: data.tk_eduser }, {upsert: false}).exec(function(err, docs){
				cb(err, docs);
			});
		}
	});
...
exports.updlstshout = function(data, cb){
	pml.findOneAndUpdate({uid: data.uid}, { lstshout: data.lstshout }, {upsert: false}).exec();
};
